...
from data_loader import get_sigloader_train,get_sigloader_test, get_sigloader #get_sigloader2, get_sigloader1 #SigLoader,
...
parser.add_argument('--net_root', default=r'E:\文献阅读\增量学习\徐云公开数据集\self_supervised_learning\第七次结果-11.13\net\resnetq.pkl', type=str, help='root of the net')
...
if __name__ == '__main__':
    ...
    for rpi in range(repeatn):
        ...
        # dataset_train = get_sigloader_train(args.data_root, args.sp_len, args.sample_len, train_transform)
        ...
        # dataset_test_total = get_sigloader_test(args.data_root, args.sp_len,args.sample_len, test_transform)
        ...
        my_net = torch.load(args.net_root).to(device)
        ...
        acc_test_total = np.zeros([args.n_epoch,])
        acc_test_old_new_net = np.zeros([args.n_epoch, ])
        acc_test_old_old_net = np.zeros([args.n_epoch, ])
        loss_test_total = np.zeros([args.n_epoch,])
        conf_test_total = np.zeros([args.class_num, args.class_num])
        #
        ...
        #
            ...
            acc_test_total[epoch], loss_test_total[epoch], conf_test_total,feats_test,labels_test = test(dataloader_test_total,my_net,args.class_num)
            acc_test_old_new_net[epoch], _, _, _, _ = test(dataloader_test_old, my_net, args.class_num)
            acc_test_old_old_net[epoch], _, _, _, _ = test(dataloader_test_old, old_net, args.class_num_old)
            ...
            if ((epoch+1) % args.save_epoch == 0):
                ...
                np.savetxt(pre_path+'/'+save_feats+'/{0}_test_feat0_{1}.txt'.format('最后结果', epoch), feats_test)
                np.savetxt(pre_path+'/'+save_feats+'/{0}_test_label0_{1}.txt'.format('最后结果', epoch), labels_test)

            print('迁移过程 epoch: {} train acc: {:.4f} train loss: {:.4f} test acc total: {:.4f} test acc old net: {:.4f} test loss difference: {:.4f}'.format(epoch,
            acc_train[epoch], loss_train[epoch], acc_test_total[epoch], acc_test_old_old_net[epoch],acc_test_old_old_net[epoch]-acc_test_old_new_net[epoch]))

        np.savetxt(pre_path + '/' + save_results + '/{}_acc_train.txt'.format('迁移过程'), acc_train)
        np.savetxt(pre_path + '/' + save_results + '/{}_loss_train.txt'.format('迁移过程'), loss_train)
        np.savetxt(pre_path + '/' + save_results + '/{}_acc_test.txt'.format('迁移过程'), acc_test_total)
        np.savetxt(pre_path + '/' + save_results + '/{}_acc_test_new_old_net.txt'.format('迁移过程'), acc_test_old_new_net)
        np.savetxt(pre_path + '/' + save_results + '/{}_acc_test_new_old_old.txt'.format('迁移过程'), acc_test_old_old_net)
        np.savetxt(pre_path + '/' + save_results + '/{}_loss_test.txt'.format('迁移过程'), loss_test_total)
        np.savetxt(pre_path + '/' + save_results + '/{}_conf_test_epoch'.format('迁移过程') + str(epoch) + '.txt', conf_test_total)
