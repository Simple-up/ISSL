...
def MultiClassCrossEntropy(logits, labels, T):#L_old的计算
...
class Model(nn.Module):
	def __init__(self, classes, classes_map, args):
		...
		self.feature_extractor = nn.DataParallel(self.feature_extractor) #多卡训练
	...
	def update(self, dataset, class_map, args):
		...
		# Save a copy to compute distillation outputs保留一份蒸馏输出的副本
		...
		if len(new_classes) > 0:
			self.increment_classes(new_classes)#初始化新的全连接层
			self.cuda()
		...
		with tqdm(total=self.num_epochs) as pbar:#加个进度条，美观
